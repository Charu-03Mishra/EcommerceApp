...
export default function Category() {
	const [isCategory, setIsCategory] = useState("Bottomset");
	...
	const selected = CategoryImages.find(
		(cat) => cat.categoryName === isCategory
	);
	...
	return (
		<SafeAreaView className="pt-4 px-2 bg-gray-200 flex-1">
			{/* Top Header */}
			<View
				className="flex flex-row items-center px-3 justify-between bg-white rounded-lg py-3"
				style={{
					shadowColor: "#000",
				...
				<Ionicons name="cart-outline" size={30} color="black"  onPress={()=> router.push("/(main)/(tab)/AddToCart")}/>
			</View>
			...
			<View className="flex flex-row flex-1 mt-2">
				...
				<View className="flex-[.3] flex-row ">
					<ScrollView
						...
						{CategoryImages.map((item, i) => (
							<TouchableOpacity
								key={i}
								onPress={() => setIsCategory(item.categoryName)}
								className="mb-4 ">
								<View
									className={`w-[55px] h-[55px] rounded-full overflow-hidden border-2 ${
										item.categoryName === isCategory
				...
				<FlatList
					...
					ListFooterComponent={
						<TouchableOpacity
							activeOpacity={0.9}
							className="bg-[#f43f5e] py-3 rounded-xl mt-2 items-center shadow">
							<Text
